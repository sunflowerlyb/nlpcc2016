...
def format_data(dir_path, tag):#根据需要是否需要给数据加标签
    if tag == 0:
        deal_data(dir_path + '/train.dat', 'train.data')#将数据处理成正确的格式
        deal_data(dir_path + '/dev.dat', 'dev.data')#
    else:
        deal_data_av(dir_path + '/train.dat', 'train.data')
        deal_data_av(dir_path + '/dev.dat', 'dev.data')
        deal_data_av(dir_path + '/test.dat', 'test.data')
...
def deal_data_av(path, files):
    ...
    generate_text(dir_path)
    file_o = open(dir_path + '/text.txt')
    content = file_o.read().decode('utf-8')
    file_o.close()

    if not os.path.exists(dir_path + '/sa.json'):
        sa = generate_sa(content)
        json.dump(sa, open(dir_path + '/sa.json', 'w'))
    else:
        sa = json.load(open(dir_path + '/sa.json','r'))

    if os.path.exists(dir_path + '/av_save.json'):
        av_save = json.load(open(dir_path + '/av_save.json','r'))
        #处理结束后会重新dump，
    ...
    for line in lines:
        ...
        if(line != ""):
            ...
            if count == 1000:
                if os.path.exists(dir_path + '/av_save.json'):
                    os.remove(dir_path + '/av_save.json')
                json.dump(av_save, open(dir_path + '/av_save.json', 'w'))
                print '.',
                count = 0
            ...
            for temp in words:
                lenword = len(temp)
                if lenword == 1:#单字词
                    file_w.write((temp[0] + '\t' + av[1][0] + '\t' + av[2][0] +  '\t' + av[3][0] + '\t' + av[4][0] + '\t' + av[5][0] + '\t' + 'S').encode('utf-8') + '\n')
                    for key in av.keys():
                        if len(av[key]) > 1:
                            av[key] = av[key][1:]
                else:#duo字词
                    for i in range(lenword):
                        if i < 4 and i < lenword -1:
                            ...
                            k = -1
                        file_w.write((temp[i] + '\t' + av[1][0] + '\t' + av[2][0] +  '\t' + av[3][0] + '\t' + av[4][0] + '\t' + av[5][0] + '\t' + dictt[k]).encode('utf-8') + '\n')
                        for key in av.keys():
                            if len(av[key]) > 1:
                                av[key] = av[key][1:]
...
def generate_text(path):
    if not os.path.exists(path + '/text.txt'):
        ...
        file_w.write(text)
        file_w.close()
        print 'generate text.txt sucessfully!'
...
if __name__ == '__main__':
    ...
    flag = 1
    tag = 1
    tt = add_tag('data', flag, tag)
    end_time = time.time()
    print("\nThe Pre process is %g seconds" % (end_time - start_time1))

    print '\n\ntag train data .',
    tt.add_all_tag('train.data')#加特征标签
    print '\n\ntag dev data .',
    tt.add_all_tag('dev.data')
    print '\n\ntag test data .',
    tt.add_all_tag('test.data')
